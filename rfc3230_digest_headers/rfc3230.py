...
class DigestHeaderAlgorithm(Enum):
    ...
    def verify_request(
        ...
        """Used by the *server* to verify a request's Digest header against the `instance` bytes.
        ...
        The `Want-Digest` header is set in the response in case of errors:
            ```
    ...
    def handle_want_digest_header(
        ...
        This calls `make_digest_header` with the algorithms that the server wants, indicated by the `want_digest_header`.
        ...
        if not wanted_digests:
            raise UnsatisfiableDigestException("No acceptable digest algorithm found.")

        return DigestHeaderAlgorithm.make_digest_header(instance, wanted_digests)

    def compute(self, data: bytes) -> str:
        """Computes the digest of the given data using the specified algorithm.
...
def _make_wants_digest_header(
    ...
    """Helper function to create the `Want-Digest` header value from the qvalues dictionary.
...
def _parse_digest_header(
    ...
    """Parses a `Digest` header into a dictionary of algorithms and their corresponding digests.
...
def _parse_want_digest_header(
    ...
    """Parses a `Want-Digest` header into a dictionary of algorithms and their corresponding qvalues.
...
def _bsdcksum(data: bytes) -> str:
    """Should be equal to `cksum` on a Unix system.
